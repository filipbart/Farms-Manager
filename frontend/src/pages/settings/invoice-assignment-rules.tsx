...
import type { InvoiceAssignmentRule } from "../../models/settings/invoice-assignment-rule";
import InvoiceAssignmentRuleModal from "../../components/modals/settings/invoice-assignment-rule-modal";
import ConfirmDialog from "../../components/common/confirm-dialog";

interface SortableRuleItemProps {
  rule: InvoiceAssignmentRule;
  onEdit: (rule: InvoiceAssignmentRule) => void;
  onDelete: (rule: InvoiceAssignmentRule) => void;
  onToggleActive: (rule: InvoiceAssignmentRule) => void;
}
    ...
    >
      ...
      <Box sx={{ flex: 1 }}>
        ...
        <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1, mb: 1 }}>
          ...
          <Chip label={rule.assignedUserName} size="small" color="primary" />
        </Box>
...
const InvoiceAssignmentRulesPage: React.FC = () => {
  const [rules, setRules] = useState<InvoiceAssignmentRule[]>([]);
  ...
  const [ruleToDelete, setRuleToDelete] =
    useState<InvoiceAssignmentRule | null>(null);
  ...
  const fetchRules = useCallback(async () => {
    ...
    try {
      await handleApiResponse(
        () => SettingsService.getInvoiceAssignmentRules(),
        (data) => {
  ...
  const handleEditRule = (rule: InvoiceAssignmentRule) => {
    setEditingRule(rule);
    setModalOpen(true);
  };

  const handleDeleteRule = (rule: InvoiceAssignmentRule) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await handleApiResponse(
        () => SettingsService.deleteInvoiceAssignmentRule(ruleToDelete.id),
        () => {
  ...
  const handleToggleActive = async (rule: InvoiceAssignmentRule) => {
    try {
      await handleApiResponse(
        () =>
          SettingsService.updateInvoiceAssignmentRule(rule.id, {
      ...
      >
        <Box>
          <Typography variant="h5" fontWeight={600}>
            Reguły przypisywania faktur
          </Typography>
          <Typography variant="body2" color="text.secondary">
            Zarządzaj regułami automatycznego przypisywania faktur do
            pracowników. Przeciągnij regułę, aby zmienić jej priorytet.
          </Typography>
        </Box>
        <Button
          variant="contained"
...
export default InvoiceAssignmentRulesPage;
