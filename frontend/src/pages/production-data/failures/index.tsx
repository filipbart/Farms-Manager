...
import {
  filterReducer,
  initialFilters,
  mapProductionDataFailureOrderTypeToField,
  ProductionDataFailureOrderType,
  type ProductionDataFailureDictionary,
} from "../../../models/production-data/failures/failures-filters";
import FiltersForm from "../../../components/filters/filters-form";
import { getProductionDataFailuresFiltersConfig } from "./filter-config.production-data-failures";
...
const ProductionDataFailuresPage: React.FC = () => {
  ...
  const [dictionary, setDictionary] =
    useState<ProductionDataFailureDictionary>();
    ...
    const fetchDictionaries = async () => {
      try {
        await handleApiResponse(
          () => ProductionDataFailuresService.getFailuresDictionaries(),
          (data) => {
            console.log(data);
  ...
  return (
    <Box p={4}>
      ...
      <FiltersForm
        config={getProductionDataFailuresFiltersConfig(
          dictionary,
          uniqueCycles,
          filters
        )}
          ...
          onSortModelChange={(model) => {
            if (model.length > 0) {
              ...
              const foundOrderBy = Object.values(
                ...
                (orderType) =>
                  mapProductionDataFailureOrderTypeToField(orderType) ===
                  sortField
              );
              dispatch({
                type: "setMultiple",
