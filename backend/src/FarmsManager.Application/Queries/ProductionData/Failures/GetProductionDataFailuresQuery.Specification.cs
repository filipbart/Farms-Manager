...
{
    public GetAllProductionDataFailuresSpec(GetProductionDataFailuresQueryFilters filters, bool withPagination)
    {
        EnsureExists();
        DisableTracking();
    ...
    private void PopulateFilters(GetProductionDataFailuresQueryFilters filters)
    {
    ...
    private void ApplyOrdering(GetProductionDataFailuresQueryFilters filters)
    {
        var isDescending = filters.IsDescending;
        switch (filters.OrderBy)
        {
            case ProductionDataFailureOrderBy.Cycle:
                if (isDescending)
                {
            ...
            case ProductionDataFailureOrderBy.Farm:
                if (isDescending)
                {
            ...
            case ProductionDataFailureOrderBy.Henhouse:
                if (isDescending)
                {
            ...
            case ProductionDataFailureOrderBy.DeadCount:
                if (isDescending)
                {
            ...
            case ProductionDataFailureOrderBy.DefectiveCount:
                if (isDescending)
                {
            ...
            case ProductionDataFailureOrderBy.DateCreatedUtc:
            default:
                if (isDescending)
                {
